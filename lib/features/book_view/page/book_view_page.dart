...
class _BookDetailState extends State<BookViewPage>{
  ...
  Widget build(BuildContext context){
    ...
    return RepositoryProvider.value(
      value: GetIt.I<BookViewRepo>(),
      child: BlocProvider(
          create: (context) => BookViewBloc(bookSignal.isbn, bookSignal.coverUrl),
          lazy: false,
          child: BlocConsumer<BookViewBloc, BookViewState>(
            listenWhen: (previous, current) => current is BookViewInitNoData || current is BookViewInitWithLocal,
            buildWhen: (previous, current) => previous != current && current is! BookRetrievedNoData,
            listener: (BuildContext context, state) => invokeReqNet(context),
            builder: (BuildContext context, state) {
              if (_firstBuild && state is BookViewInitNoData || state is BookViewInitWithLocal){
                          ...
                          child: BoxGroove(
                           title: '更多${bookInfo.authorNames[0]}作品',
                            widgets: List.generate(bookInfo.relatedBooks!.length,
                              (index) => ImageInfoBox(
                                image: Image.network(
                                  bookInfo.relatedBooks![index].coverSUrl,
                                  fit: BoxFit.cover,
                                  width: 130,
                                  height: 180,
                                ),
                                title: bookInfo.relatedBooks![index].title,
